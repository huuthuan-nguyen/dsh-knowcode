...
export class TreeSitterEngine {
   ...
   */
  public static normalizeLanguageName(name: string): string {
    const lower = name.toLowerCase().replace(/[\s-]/g, '_');
    if (lower === 'csharp' || lower === 'cs' || lower === 'c#') return 'c_sharp';
   ...
   */
  private static scanCustomGrammarDirs(): void {
    const dirs = [
   ...
   */
  public static async ensureLanguage(lang: string): Promise<boolean> {
    const norm = TreeSitterEngine.normalizeLanguageName(lang);
    ...
    const promise = (async () => {
      ...
      try {
        ...
        if (norm !== lang) this.languages.set(lang, language);
   ...
   */
  public static async ensureLanguages(langs: Iterable<string>): Promise<void> {
   ...
   */
  public static async registerGrammar(
    lang: string,
   ...
   */
  public static registerExtension(extension: string, language: string): void {
    const ext = extension.startsWith('.') ? extension.toLowerCase() : `.${extension.toLowerCase()}`;
    this.customExtensions.set(ext, TreeSitterEngine.normalizeLanguageName(language));
  }

  public static getCustomExtension(extension: string): string | undefined {
  ...
  public static isLanguageLoaded(lang: string): boolean {
    const norm = TreeSitterEngine.normalizeLanguageName(lang);
    return this.parsers.has(norm) || this.parsers.has(lang);
  }

  public static getLanguage(lang: string): Parser.Language | undefined {
    const norm = TreeSitterEngine.normalizeLanguageName(lang);
    return this.languages.get(norm) ?? this.languages.get(lang);
  }

  public static getParser(lang: string): Parser | undefined {
    const norm = TreeSitterEngine.normalizeLanguageName(lang);
   ...
   */
  public static parse(language: string, source: string, filePath?: string): Parser.Tree | null {
