 ...
 */
export function signatureFrom(text: string): string {
 ...
 */
function extractDocstring(node: Parser.SyntaxNode, lines: string[]): string | undefined {
...
export function parseWithTreeSitterAst(
  filePath: string,
  language: string,
  source: string,
  lines: string[],
  tree: Parser.Tree,
  resolveCandidates: (file: string, path: string) => string[]
          ...
          if (child.type === 'import_clause') {
            for (const sub of child.children) {
              if (sub.type === 'identifier') {
                ...
                if (idNode) specifiers.push(idNode.text);
              }
            }
          }
        }
...
function visitClassMember(
  ...
  if (member.type === 'method_definition') {
    ...
    if (!nameNode) return;
    const name = nameNode.text;
    if (NON_DECLARATION_KEYWORDS.has(name)) return;
...
function extractPythonDocstring(node: Parser.SyntaxNode): string | undefined {
  const body = node.childForFieldName('body');
  if (body) {
    ...
    if (node.type === 'type_declaration') {
      for (const spec of node.children.filter((c) => c.type === 'type_spec')) {
        ...
        if (nameNode) {
          ...
          const startLine = node.startPosition.row + 1;
          const endLine = node.endPosition.row + 1;
          const kind: SymbolKind = typeNode?.type === 'struct_type' ? 'struct' : typeNode?.type === 'interface_type' ? 'interface' : 'type';
          const sig = lines[startLine - 1]?.trim() ?? `type ${name}`;
...
function parseCSharpAst(
  ...
  function visit(node: Parser.SyntaxNode, currentContainer: string | null = null): void {
    ...
    if (
      ...
      if (nameNode) {
        ...
        const kind: SymbolKind = node.type === 'interface_declaration' ? 'interface' : node.type === 'struct_declaration' ? 'struct' : 'class';
        symbols.push({
          id: symId,
          name,
          qname,
    ...
    if (node.type === 'class_declaration' || node.type === 'interface_declaration') {
      ...
      if (nameNode) {
        ...
        if (baseClause) {
          ...
          if (superName) heritage.push({ subSymbolId: symId, superSymbolName: superName, kind: 'extends' });
        }
...
function parseSolidityAst(
  ...
  function visit(node: Parser.SyntaxNode, currentContract: string | null = null): void {
...
function parseUniversalAst(
  ...
  function visit(node: Parser.SyntaxNode): void {
    ...
    if (t.includes('class') || t.includes('record')) kind = 'class';
    else if (t.includes('interface') || t.includes('protocol')) kind = 'interface';
    else if (t.includes('trait')) kind = 'trait';
    else if (t.includes('struct')) kind = 'struct';
    else if (t.includes('enum')) kind = 'enum';
    else if (t.includes('module') || t.includes('namespace')) kind = 'module';
    ...
    if (kind && (isContainer || isCallable)) {
      ...
      if (!name) {
        const idChild = node.children.find(
          (c) =>
            (c.type.includes('ident') || c.type.includes('name')) &&
