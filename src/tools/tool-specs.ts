import { emptySchema, objectSchema, type JsonSchemaObject } from './json-schema.js';
...
export const knowCodeToolSpecs: readonly KnowCodeToolSpec[] = [
  {
  ...
  },
  {
    ...
    description:
      'PRIMARY ORIENTATION TOOL for exploring unfamiliar repositories and large codebases. Queries the FalkorDB code graph to return top-level entry points (exported symbols with 0 internal callers), central hub symbols (ranked by incoming call in-degree / PageRank), and language breakdown without flooding the LLM context window with raw source code.',
  ...
  },
  {
    ...
    description:
      'Retrieve the exact AST definition, complete type signature, parameter list, return type, docstring, file path, and start/end line numbers of any function, method, class, struct, or interface. Eliminates signature hallucinations and argument guessing during implementation or interface conformance.',
    parameters: objectSchema(
      {
        name: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Find all direct invocation sites calling a specific function or class method across the entire codebase. Returns caller names, qualified caller identifiers, file paths, and exact line numbers. Essential during refactoring to update every invocation site when modifying function parameters, argument orders, or return types.',
    parameters: objectSchema(
      {
        symbol: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Find all subroutines, helper functions, and methods called directly by a specific function or method (downward execution trace). Inspects dependencies, sub-calls, and error-checking branches to understand implementation logic and preconditions before modifying a function.',
    parameters: objectSchema(
      {
        symbol: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Discover all automated unit and integration test files that cover or import the specified source files. Uses both graph import dependencies and naming conventions (*.test.ts, *_test.go, test_*.py). Run this after completing code edits or bug fixes to verify zero regressions.',
    parameters: objectSchema(
      {
        files: {
          type: 'array',
  ...
  },
  {
    ...
    description:
      'CROSS-LANGUAGE PORTING ACCELERATOR: Extracts the complete language-agnostic interface blueprint of a class, struct, or module: exported signatures, member fields, enclosed methods, external callee dependencies, governing ADR architectural rules, and test suites. Provides the exact specification needed to port code to another language (e.g. Python to Go, TypeScript to Rust) with 100% semantic fidelity.',
    parameters: objectSchema(
      {
        symbol: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Search across project documentation, Architecture Decision Records (ADRs), specifications, and coding standards. Returns matched sections with priority levels (MUST / SHOULD / AVOID) and automatic cross-links to related code symbols in the graph.',
    parameters: objectSchema(
      {
        query: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Retrieve the full content, heading hierarchy, and architectural rules for a specific documentation file or ADR title. Use this to understand why an architectural decision was made (the "Why") before refactoring existing code patterns.',
    parameters: objectSchema(
      {
        pathOrTitle: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'ADVANCED ESCAPE HATCH: Execute a raw Cypher query directly against the FalkorDB code and knowledge graph for specialized multi-hop traversals not covered by standard tools (e.g. finding classes implementing an interface that do not call a specific validator).',
    parameters: objectSchema(
      {
        cypher: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Trace the shortest call invocation path between two functions or methods across the entire codebase graph (up to 12 hops). Traces multi-hop execution flow from entry points to downstream services, helping verify architectural layering, security boundaries, and dataflow paths.',
    parameters: objectSchema(
      {
        fromSymbol: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Find all subclasses, structs, and classes that extend or implement a given interface, abstract class, or base class. Essential for polymorphism, dependency injection, and factory pattern refactorings.',
    parameters: objectSchema(
      {
        symbol: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'Search for symbols across the codebase by name pattern, case-insensitive substring, or keyword. Useful when you only know a partial name or concept (e.g. "Token", "Auth", "Payment") and need to locate the exact qualified symbol name.',
    parameters: objectSchema(
      {
        pattern: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'ONE-CLICK REGRESSION AUDIT: Analyzes uncommitted git working tree changes (or changes against a base commit/branch), maps changed line ranges to AST symbols, computes transitive callers at risk, and identifies all automated test suites to run before commit or PR creation.',
    parameters: objectSchema({
      baseRef: {
        type: 'string',
  ...
  },
  {
    ...
    description:
      'CROSS-PARADIGM PORTING ACCELERATOR: Bidirectional paradigm transformation tool. Supports OOP (Java/C#) ➔ Systems/Functional (Rust/Go) by flattening class hierarchies into traits/structs and Result/Error enums. Also supports reverse porting: Functional/Systems (Rust/Go) ➔ OOP (Java/C#) with encapsulation, getters/setters, interface conformance, and class exception hierarchies.',
    parameters: objectSchema(
      {
        symbol: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'ZERO-DEPENDENCY POLYFILL & ECOSYSTEM RECOMMENDER: Analyzes the exact subset of an external 3rd-party library actually invoked by your codebase. Eliminates 95% of unused surface area, outputs a minimal functional specification to re-implement from scratch, AND recommends equivalent target ecosystem libraries (e.g. Java Guava Cache ➔ Rust moka or Go bigcache) with method-to-method API mappings.',
    parameters: objectSchema(
      {
        libraryPrefix: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'BIDIRECTIONAL TRACEABILITY (Code ➔ Feature Specs): Identifies all features, specifications in *.md, PRD sections, and architectural rules that a specific function, method, or class belongs to or serves. Traces both direct documentation links and upstream caller execution paths.',
    parameters: objectSchema(
      {
        symbol: {
          type: 'string',
  ...
  },
  {
    ...
    description:
      'POLYGLOT STORAGE MIGRATION BLAST RADIUS: Analyzes the breaking impact of altering, dropping, or renaming an attribute in SQL tables, MongoDB collections, Elasticsearch indices, or Vector DB collections. Detects all affected external contracts (XML/JSON/gRPC), application DAOs/queries, and mapped test suites.',
    parameters: objectSchema(
      {
        storageContainer: {
          type: 'string',
