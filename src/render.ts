...
export function renderExplore(data: {
  ...
  const parts: string[] = ['### 🧭 Codebase Architectural Overview\n'];
...
export function renderBlastRadius(data: BlastRadiusResult): string {
  const parts: string[] = [];
  const riskBadge = {
    low: '🟢 LOW RISK',
    medium: '🟡 MEDIUM RISK',
...
export function renderCallers(
  symbol: string,
  callers: Array<{ callerName: string; callerQName: string; callerKind: string; file: string; line: number }>
): string {
  if (callers.length === 0) {
...
export function renderCallees(
  symbol: string,
  callees: Array<{ calleeName: string; calleeQName: string; calleeKind: string; file: string }>
): string {
  if (callees.length === 0) {
...
export function renderCycles(data: CycleDetectionResult): string {
  if (!data.hasCycles) {
...
export function renderSymbolDefinition(sym: CodeSymbol | null, name: string): string {
  if (!sym) {
...
export function renderPortingContract(data: PortingContractResult | null, symbol: string): string {
  if (!data) return `Could not generate porting contract for \`${symbol}\`. Symbol not found.`;
  ...
  if (data.dependencies.callees.length === 0) {
    lines.push('_Pure self-contained logic (zero external callees). Ideal for porting!_');
  } else {
...
export function renderStatus(stats: KnowCodeStats): string {
  return [
...
export function renderCallPath(data: CallPathResult): string {
  if (!data.found) {
...
export function renderSubtypes(symbol: string, subtypes: SubtypeResult[]): string {
...
export function renderSymbolSearchResults(pattern: string, symbols: CodeSymbol[]): string {
  if (symbols.length === 0) {
...
export function renderUnusedSymbols(symbols: UnusedSymbolResult[]): string {
  if (symbols.length === 0) {
...
export function renderGitDiffImpact(data: GitDiffImpactResult): string {
  const riskBadge =
...
export function renderCrossParadigmBlueprint(data: CrossParadigmBlueprint): string {
  const icon =
    data.targetLanguage === 'rust'
      ? '🦀'
...
export function renderThirdPartyUsageSlice(data: ThirdPartyUsageSlice): string {
  const lines = [
...
export function renderDuplicateClones(clones: DuplicateCloneResult[]): string {
  if (clones.length === 0) {
...
export function renderFeatureCodeFlow(data: FeatureCodeFlowResult): string {
...
export function renderSymbolSpecFeatures(data: SymbolSpecFeaturesResult): string {
  const lines = [
...
export function renderContractStorageMapping(data: ContractStorageMappingResult): string {
  const engineBadge =
    data.storageEngine === 'sql'
      ? '🐘 SQL (RDBMS)'
  ...
  if (data.mappings.length > 0) {
    ...
    for (const m of data.mappings) {
      const statusBadge =
        m.typeStatus === 'compatible'
          ? '✅ Compatible'
          : m.typeStatus === 'conversion_required'
          ? '🔄 Conversion'
          : '⚠️ Discrepancy';
      lines.push(
        `| \`${m.contractField}\` | \`${m.contractType}\` | \`${m.storageAttribute}\` | \`${m.storageType}\` | ${m.role} | ${statusBadge} | ${m.note ?? '-'} |`
      );
    }
  }
...
export function renderStorageMigrationImpact(data: StorageMigrationImpactResult): string {
  const riskBadge =
