...
import type {
  ...
  UnusedSymbolResult,
  GitDiffImpactResult,
  CrossParadigmBlueprint,
  ThirdPartyUsageSlice,
  DuplicateCloneResult,
  FeatureCodeFlowResult,
  FeatureCodeFlowNode,
  SymbolSpecFeaturesResult,
  ContractEntityDef,
  StorageContainerDef,
  ContractStorageMappingResult,
  StorageMigrationImpactResult,
  StorageEngine,
  FieldStorageMapping,
} from '../types.js';
...
const ECOSYSTEM_KNOWLEDGE_BASE: Record<
  string,
  {
    category: string;
    equivalents: Record<string, { pkg: string; install: string; desc: string }>;
  }
> = {
  cache: {
...
export class KnowCodeRepository {
   ...
   */
  public async ingestCalls(calls: CodeCall[]): Promise<void> {
    for (const call of calls) {
      ...
      const matched = (res.data?.[0] as any)?.matched ?? 0;
      if (matched === 0) {
    ...
    for (const sec of doc.sections) {
      ...
      if (sec.referencedSymbols) {
        for (const symName of sec.referencedSymbols) {
          await this.graph.query(
            `MATCH (s:DocSection {id: $secId})
             MATCH (sym:Symbol)
             WHERE sym.name = $symName OR sym.qname = $symName
             MERGE (s)-[:DOCUMENTS]->(sym)`,
            { params: { secId: sec.id, symName } }
          );
        }
      }
    }
   ...
   */
  public async explore(limit: number = 25): Promise<{
    files: Array<{ path: string; language: string; lineCount: number }>;
    topSymbols: Array<{ name: string; kind: string; file: string; inDegree: number }>;
    entryPoints: Array<{ name: string; file: string; kind: string }>;
  }> {
    // 1. Files overview
    const filesRes = await this.graph.query(
      `MATCH (f:File) RETURN f.path AS path, f.language AS language, f.lineCount AS lineCount LIMIT 20`
    );
    const files = (filesRes.data ?? []).map((row: any) => ({
      path: row.path,
    ...
    const hubsRes = await this.graph.query(
    ...
    const topSymbols = (hubsRes.data ?? []).map((row: any) => ({
      name: row.name,
      kind: row.kind,
      file: row.file,
   ...
   */
  public async getCallers(symbolName: string): Promise<
    Array<{
      callerName: string;
      callerQName: string;
      callerKind: string;
  ...
  > {
    const res = await this.graph.query(
      `MATCH (caller:Symbol)-[r:CALLS]->(callee:Symbol)
       ...
       RETURN caller.name AS callerName, caller.qname AS callerQName, caller.kind AS callerKind, caller.file AS file, r.line AS line`,
      { params: { symbolName } }
    );
   ...
   */
  public async getCallees(symbolName: string): Promise<
    Array<{
      calleeName: string;
      calleeQName: string;
      calleeKind: string;
  ...
  > {
    const res = await this.graph.query(
      `MATCH (caller:Symbol)-[:CALLS]->(callee:Symbol)
       ...
       RETURN callee.name AS calleeName, callee.qname AS calleeQName, callee.kind AS calleeKind, callee.file AS file`,
      { params: { symbolName } }
    );
    return (res.data ?? []).map((row: any) => ({
   ...
   */
  public async getBlastRadius(symbolName: string, maxDepth: number = 3): Promise<BlastRadiusResult> {
    ...
    const callChain: BlastRadiusResult['callChain'] = [];
  ...
  public async detectCycles(): Promise<CycleDetectionResult> {
    const res = await this.graph.query(
      `MATCH path = (f1:File)-[:IMPORTS*2..6]->(f1)
    ...
    for (const row of (res.data ?? []) as any[]) {
      const nodes: string[] = row.cycleNodes;
   ...
   */
  public async getAffectedTests(sourceFiles: string[]): Promise<string[]> {
    const testFiles = new Set<string>();

    for (const src of sourceFiles) {
      // 1. By naming convention (e.g. foo.ts -> foo.test.ts or test_foo.py)
      const baseName = src.replace(/\.[^/.]+$/, '');
      const namingRes = await this.graph.query(
        `MATCH (f:File {isTest: true})
         WHERE f.path CONTAINS $baseName
         RETURN f.path AS path`,
        { params: { baseName } }
      );
      for (const r of (namingRes.data ?? []) as any[]) {
        testFiles.add(r.path);
      }

      // 2. By import relationship: test file imports source file
      const importRes = await this.graph.query(
        `MATCH (tf:File {isTest: true})-[:IMPORTS]->(sf:File {path: $src})
         RETURN tf.path AS path`,
        { params: { src } }
      );
      for (const r of (importRes.data ?? []) as any[]) {
        testFiles.add(r.path);
      }
    }
   ...
   */
  public async getPortingContract(symbolName: string): Promise<PortingContractResult | null> {
    ...
    const typesAndMembers = (membersRes.data ?? []).map((r: any) => ({
      name: r.name,
      kind: r.kind,
      signature: r.signature,
      docstring: r.docstring,
    ...
    const rulesRes = await this.graph.query(
      `MATCH (r:Rule)
       WHERE r.content CONTAINS $name OR r.title CONTAINS $name
       RETURN r.title AS title, r.content AS content`,
      { params: { name: sym.name } }
    );
    const governingRules = (rulesRes.data ?? []).map((r: any) => ({
      title: r.title,
      content: r.content,
    }));

    // Supertypes are a porting dependency in their own right: a class that implements
    // an interface cannot be moved without it.
    const superRes = await this.graph.query(
      `MATCH (s:Symbol {name: $name})-[:EXTENDS|IMPLEMENTS]->(sup)
       RETURN sup.name AS name, labels(sup) AS labels`,
      { params: { name: sym.name } }
    );
    const supertypes = (superRes.data ?? []).map((r: any) => ({
      name: r.name,
      external: Array.isArray(r.labels) && r.labels.includes('ExternalType'),
    }));
   ...
   */
  public async searchKnowledge(query: string, limit: number = 10): Promise<
    Array<{
  ...
  > {
    ...
    // Search doc sections
    const secRes = await this.graph.query(
      `MATCH (sec:DocSection)
    ...
    for (const r of (secRes.data ?? []) as any[]) {
      results.push({
        type: 'section',
        title: r.title,
    ...
    // Search rules
    const ruleRes = await this.graph.query(
      `MATCH (r:Rule)
    ...
    for (const r of (ruleRes.data ?? []) as any[]) {
      results.push({
        type: 'rule',
        title: r.title,
        snippet: r.content,
   ...
   */
  public async getKnowledgeDoc(pathOrTitle: string): Promise<{
    title: string;
    path: string;
    category: string;
    sections: Array<{ title: string; content: string; level: number }>;
    rules: Array<{ title: string; content: string; priority: string }>;
  } | null> {
    // Resolution order: exact path, exact title, then a path suffix. The tool
    // promises "path or title", and `spec.md` is how a caller naturally refers to
    // `docs/spec.md` — an exact match alone reported it as missing.
    const docRes = await this.graph.query(
      `MATCH (d:Document)
       WHERE d.path = $query OR d.title = $query OR d.path ENDS WITH ('/' + $query)
       RETURN d.title AS title, d.path AS path, d.category AS category
       ORDER BY CASE WHEN d.path = $query THEN 0 WHEN d.title = $query THEN 1 ELSE 2 END
       LIMIT 1`,
      { params: { query: pathOrTitle } }
    );

    const docData = docRes.data as any[];
    if (!docData || docData.length === 0) return null;
    const doc = docData[0];

    const secRes = await this.graph.query(
      `MATCH (d:Document {path: $path})-[:HAS_SECTION]->(s:DocSection)
    ...
    const ruleRes = await this.graph.query(
      `MATCH (d:Document {path: $path})-[:HAS_RULE]->(r:Rule)
       RETURN r.title AS title, r.content AS content, r.priority AS priority`,
      { params: { path: doc.path } }
    );
    ...
    return {
      ...
      path: doc.path,
      category: doc.category,
      sections: ((secRes.data ?? []) as any[]).map((s: any) => ({
        title: s.title,
        content: s.content,
        level: s.level,
      })),
      rules: ((ruleRes.data ?? []) as any[]).map((r: any) => ({
        title: r.title,
        content: r.content,
        priority: r.priority,
      })),
  ...
  public async getStats(): Promise<{
    ...
    for (const row of (fRes.data ?? []) as any[]) {
      totalFiles += row.cnt;
      if (row.lang) {
   ...
   */
  public async findCallPath(fromSymbol: string, toSymbol: string): Promise<CallPathResult> {
    const res = await this.graph.query(
      `MATCH path = (src:Symbol)-[:CALLS*1..12]->(dst:Symbol)
    ...
    const path: CallPathNode[] = rawNodes.map((n: any) => {
  ...
  public async findSubtypesAndImplementations(symbolName: string): Promise<SubtypeResult[]> {
    const res = await this.graph.query(
      `MATCH (sub:Symbol)-[r:EXTENDS|IMPLEMENTS]->(target)
       ...
       ORDER BY sub.file, sub.startLine`,
   ...
   */
  public async analyzeGitDiffImpact(workdir: string, baseRef?: string): Promise<GitDiffImpactResult> {
    ...
    const impactedSymbols: GitDiffImpactResult['impactedSymbols'] = [];
    let totalTransitiveCallers = 0;

    for (const diff of diffs) {
      const symRes = await this.graph.query(
        `MATCH (s:Symbol {file: $file})
         RETURN s.name AS name, s.qname AS qname, s.kind AS kind, s.file AS file,
                s.startLine AS startLine, s.endLine AS endLine`,
        { params: { file: diff.file } }
      );

      for (const row of (symRes.data ?? []) as any[]) {
        const start = row.startLine ?? 0;
        const end = row.endLine ?? start;

        const overlaps =
          diff.changedLines.length === 0 ||
          diff.changedLines.some(
            (r) =>
        ...
        if (overlaps) {
          ...
          const callerRes = await this.graph.query(
            `MATCH ()-[:CALLS]->(s:Symbol {name: $name, file: $file}) RETURN count(*) AS cnt`,
            { params: { name: row.name, file: row.file } }
          );
          const cCnt = (callerRes.data?.[0] as any)?.cnt ?? 0;
   ...
   */
  public async generateCrossParadigmBlueprint(
    symbolName: string,
    targetLanguage: 'rust' | 'go' | 'java' | 'csharp' = 'rust'
    ...
    const membersRes = await this.graph.query(
      `MATCH (c:Symbol)-[:CONTAINS]->(m:Symbol)
       WHERE c.name = $name OR c.qname = $name
       RETURN m.name AS name, m.kind AS kind, m.signature AS signature, m.docstring AS docstring`,
      { params: { name: symbolName } }
    );

    const members = (membersRes.data ?? []) as any[];
    const supRows = (supertypes.data ?? []) as any[];

    const structs: CrossParadigmBlueprint['structs'] = [];
    const traits: CrossParadigmBlueprint['traits'] = [];
    const errorEnums: CrossParadigmBlueprint['errorEnums'] = [];
    const ownershipGuidelines: string[] = [];

    // Main struct / class fields
    const fields: Array<{ name: string; type: string; optional: boolean }> = [];
    const methods: Array<{ name: string; signature: string; isMut: boolean }> = [];
    ...
    if (targetLanguage === 'rust') {
      ...
      const traitBlocks = traits
        .map(
          (t) =>
            `pub trait ${t.name} {\n` +
            `    // TODO: copy the member signatures of \`${t.name}\` here.\n` +
            `}\n`
        )
        .join('\n');
      const traitImpls = traits
        .map(
          (t) =>
            `impl ${t.name} for ${structName} {\n` +
            `    // TODO: implement the members declared by \`${t.name}\`.\n` +
            `}\n`
        )
        .join('\n');
      ...
      idiomaticSkeleton =
        ...
        (traitBlocks.length > 0 ? traitBlocks + '\n' : '') +
      ...
      ownershipGuidelines.push(
        'FLATTEN INHERITANCE: Go uses struct embedding and implicit interfaces. Define interfaces where consumed, not where implemented.',
        'ERROR HANDLING: Return `(T, error)` tuples explicitly rather than throwing exceptions.',
        'POINTER RECEIVERS: Use pointer receivers `func (s *' + (sym?.name ?? symbolName) + ')` for methods modifying state.'
      );
      idiomaticSkeleton =
      ...
      ownershipGuidelines.push(
        'ENCAPSULATION: Wrap data members in private fields with getters and setters, or use Java 17+ Records for immutable DTOs.',
        'CONVERT TRAITS TO INTERFACES: Convert Rust traits or Go interfaces to explicit Java `public interface` and `implements` clauses.',
        'EXCEPTION HIERARCHY: Convert functional `Result<T, E>` or Go `error` returns to checked/unchecked Java exceptions (`throws ${structName}Exception`).',
        'POLYMORPHIC ENUMS: Convert Rust algebraic enums to Java 17+ `sealed interface` with `permits` or standard Polymorphic class hierarchies.'
      );
      ...
      ownershipGuidelines.push(
        'PROPERTIES & INTERFACES: Use auto-implemented properties `{ get; set; }` and explicit interface inheritance with `:` syntax.',
        'NULLABLE REFERENCE TYPES: Enable C# nullable reference types (`string?`) to capture Rust `Option<T>` safety.'
      );
   ...
   */
  public async extractThirdPartyUsageSlice(
    libraryPrefix: string,
    targetLanguage: string = 'rust'
  ): Promise<ThirdPartyUsageSlice> {
    const importRes = await this.graph.query(
      `MATCH (f:File)-[r:IMPORTS]->(target)
       WHERE r.importedPath CONTAINS $prefix OR target.path CONTAINS $prefix
       RETURN r.importedPath AS path, r.specifiers AS specifiers, f.path AS filePath`,
      { params: { prefix: libraryPrefix } }
    );

    const importedSymbolsSet = new Set<string>();
    ...
    for (const row of (importRes.data ?? []) as any[]) {
      ...
      if (typeof row.specifiers === 'string') {
        for (const s of row.specifiers.split(',')) {
          ...
          if (trimmed && trimmed !== '*') importedSymbolsSet.add(trimmed);
        }
      } else if (Array.isArray(row.specifiers)) {
        for (const s of row.specifiers) {
          if (s !== '*') importedSymbolsSet.add(s);
        }
      }
    }
    ...
    const invokedMethods: ThirdPartyUsageSlice['invokedMethods'] = Array.from(methodMap.entries()).map(([name, val]) => ({
      name,
      qname: val.qname,
    ...
    const recommendedPackages: ThirdPartyUsageSlice['recommendedPackages'] = [];
    ...
    if (matchedCategoryKey) {
      ...
      if (targetEq) {
        recommendedPackages.push({
          ecosystem:
            targetLanguage.toLowerCase() === 'rust'
              ? 'crates.io'
   ...
   */
  public async findSimilarFunctions(options?: {
    threshold?: number;
    targetSymbol?: string;
    minLines?: number;
    ...
    const params: Record<string, any> = { minLines };
    ...
    const res = await this.graph.query(cypher, { params });
    const results: DuplicateCloneResult[] = [];
   ...
   */
  public async findCodeFlowForFeature(
    query: string,
    flowDepth: number = 3
  ): Promise<FeatureCodeFlowResult> {
    ...
    let secRow = (secRes.data?.[0] as any) ?? null;
    ...
    if (!secRow) {
      ...
      secRow = (docRes.data?.[0] as any) ?? null;
    }
    ...
    const entryRes = await this.graph.query(
      `MATCH (sec:DocSection {id: $secId})
       ...
       RETURN DISTINCT sym.name AS name, sym.qname AS qname, sym.kind AS kind, sym.file AS file, sym.startLine AS line`,
      { params: { secId: secRow.id } }
    );

    const entrySymbols: FeatureCodeFlowResult['entrySymbols'] = ((entryRes.data ?? []) as any[]).map((r) => ({
      name: r.name,
      qname: r.qname ?? r.name,
      kind: r.kind,
      file: r.file,
    ...
    const executionFlow: FeatureCodeFlowNode[] = [];
    const filesSet = new Set<string>();
    const visitedFlowKeys = new Set<string>();
    const mermaidEdges: Array<{ from: string; to: string }> = [];
    ...
    for (const entry of entrySymbols) {
      ...
      for (const row of (pathRes.data ?? []) as any[]) {
        const pNodes = row.pathNodes as any[];
   ...
   */
  public async findSpecFeaturesForSymbol(
    symbolName: string,
    searchCallers: boolean = true
  ): Promise<SymbolSpecFeaturesResult> {
    const sym = await this.getSymbolDefinition(symbolName);

    // 1. Direct documentation links
    const directRes = await this.graph.query(
      `MATCH (sec:DocSection)-[:DOCUMENTS]->(s:Symbol)
       ...
       WHERE s.name = $name OR s.qname = $name
       RETURN r.sourceDoc AS specFile, r.title AS sectionTitle, r.content AS content, 'governs' AS rel`,
      { params: { name: symbolName } }
    );

    const directSpecs: SymbolSpecFeaturesResult['directSpecs'] = ((directRes.data ?? []) as any[]).map((r) => ({
      specFile: r.specFile,
    ...
    const indirectFeatures: SymbolSpecFeaturesResult['indirectFeatures'] = [];
    if (searchCallers) {
      const upRes = await this.graph.query(
        `MATCH path = (root:Symbol)-[:CALLS*1..4]->(target:Symbol)
         WHERE (target.name = $name OR target.qname = $name)
      ...
      for (const row of (upRes.data ?? []) as any[]) {
   ...
   */
  public async ingestContractEntity(entity: ContractEntityDef): Promise<void> {
    const entityId = `${entity.file}:${entity.name}`;
    await this.graph.query(
      `MERGE (c:ContractEntity {id: $id})
    ...
    for (const f of entity.fields) {
      ...
      await this.graph.query(
        `MATCH (c:ContractEntity {id: $entityId})
   ...
   */
  public async ingestStorageContainer(container: StorageContainerDef): Promise<void> {
    const containerId = `${container.file}:${container.name}:${container.engine}`;
    await this.graph.query(
      `MERGE (sc:StorageContainer {id: $id})
    ...
    for (const a of container.attributes) {
      ...
      await this.graph.query(
        `MATCH (sc:StorageContainer {id: $containerId})
   ...
   */
  public async schemaMapContractToStorage(
    contractName: string,
    storageTarget?: string,
    engine?: StorageEngine
  ): Promise<ContractStorageMappingResult> {
    // 1. Locate ContractEntity
    const entityRes = await this.graph.query(
      `MATCH (c:ContractEntity)
    ...
    const cRow = (entityRes.data?.[0] as any) ?? null;
    if (!cRow) {
      return {
    ...
    const fieldsRes = await this.graph.query(
      `MATCH (c:ContractEntity {id: $id})-[:HAS_FIELD]->(cf:ContractField)
       RETURN cf.name AS name, cf.rawType AS rawType, cf.isRequired AS isRequired, cf.isList AS isList, cf.tagNumber AS tagNumber`,
      { params: { id: cRow.id } }
    );
    const contractFields = ((fieldsRes.data ?? []) as any[]).map((f) => ({
      name: f.name,
      rawType: f.rawType,
    ...
    let scRow = (scRes.data?.[0] as any) ?? null;
    ...
    if (!scRow) {
      ...
      scRow = (fallbackRes.data?.[0] as any) ?? null;
    }
    ...
    const attrRes = await this.graph.query(
      `MATCH (sc:StorageContainer {id: $id})-[:HAS_ATTRIBUTE]->(sa:StorageAttribute)
       RETURN sa.name AS name, sa.dataType AS dataType, sa.attributeRole AS attributeRole, sa.isNullable AS isNullable, sa.vectorDimension AS vectorDimension`,
      { params: { id: scRow.id } }
    );
    const storageAttrs = ((attrRes.data ?? []) as any[]).map((a) => ({
      name: a.name,
      dataType: a.dataType,
    ...
    const mappings: FieldStorageMapping[] = [];
    const matchedContractFieldSet = new Set<string>();
    const matchedStorageAttrSet = new Set<string>();

    for (const cf of contractFields) {
      let bestMatch: { attr: any; match: ReturnType<typeof StorageParser.matchFieldToAttribute> } | null = null;
      for (const sa of storageAttrs) {
        const matchResult = StorageParser.matchFieldToAttribute(cf, sa, scRow.engine as StorageEngine);
      ...
      if (bestMatch) {
        ...
        mappings.push({
          ...
          storageEngine: scRow.engine as StorageEngine,
   ...
   */
  public async schemaAnalyzeStorageMigrationImpact(
    containerName: string,
    attributeName: string,
    action: 'drop' | 'rename' | 'type_change' = 'drop'
  ): Promise<StorageMigrationImpactResult> {
    const scRes = await this.graph.query(
      `MATCH (sc:StorageContainer)
       WHERE sc.name = $cName OR sc.name CONTAINS $cName
       RETURN sc.name AS name, sc.engine AS engine, sc.file AS file
       LIMIT 1`,
      { params: { cName: containerName } }
    );

    const scRow = (scRes.data?.[0] as any) ?? {
      name: containerName,
      engine: 'sql',
      file: 'schema.sql',
    };
    ...
    const contractRes = await this.graph.query(
      `MATCH (c:ContractEntity)-[:HAS_FIELD]->(cf:ContractField)
       RETURN c.name AS entityName, c.format AS format, c.file AS file, cf.name AS fieldName`
    );

    const impactedContracts: StorageMigrationImpactResult['impactedContracts'] = [];
    for (const row of (contractRes.data ?? []) as any[]) {
    ...
    const codeRes = await this.graph.query(
      `MATCH (s:Symbol)
    ...
    const impactedCodeSymbols: StorageMigrationImpactResult['impactedCodeSymbols'] = ((codeRes.data ?? []) as any[]).map((r) => ({
      name: r.name,
      qname: r.qname ?? r.name,
      file: r.file,
      line: r.line,
    ...
    const riskLevel: StorageMigrationImpactResult['riskLevel'] =
      impactedContracts.length > 2 || action === 'drop'
        ? 'critical'
    ...
    if (action === 'drop') {
      recommendations.push(
        `DEPRECATION FIRST: Do not immediately DROP '${attributeName}'. Mark the field '@deprecated' in ${impactedContracts.length} external contract(s) and allow client migration window.`,
        `BACKWARD COMPATIBILITY: If clients still query this attribute, introduce a computed virtual column or backward-compatible view.`
      );
    } else if (action === 'type_change') {
      recommendations.push(
        `TYPE MIGRATION: Ensure deserializers in ${impactedContracts.length} contract(s) support progressive data conversion without throwing runtime unmarshal exceptions.`
      );
    }
