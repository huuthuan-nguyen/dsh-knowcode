...
export class FalkorDBManager {
   ...
   */
  public static async findOpenPort(startPort: number = 6380): Promise<number> {
    for (let port = startPort; port < startPort + 100; port++) {
      const isAvailable = await new Promise<boolean>((res) => {
   ...
   */
  public static resolveBinaries(): { redisServerPath: string; modulePath: string } | null {
  ...
  public async start(options: {
    ...
    if (!binaries) {
      if (process.platform === 'win32') {
        throw new Error(
          `[KnowCode] Embedded FalkorDB binaries are not supported natively on Windows (Redis 7.2+ and GraphBLAS require a POSIX environment).\n` +
      ...
      throw new Error(
        `[KnowCode] Embedded FalkorDB binaries not found for platform: ${process.platform}-${process.arch}.\n` +
  ...
  public getInstance(): FalkorInstance | null {
    return this.instance;
  }
}
