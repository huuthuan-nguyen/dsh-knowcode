 ...
 */
export async function executeKnowCodeTool(rawAction, args, workdir, daemonPort = 48123, options = {}) {
    const state = { startedNote: '' };
    const result = await executeKnowCodeToolCore(rawAction, args, workdir, daemonPort, options, state);
    if (state.startedNote.length === 0)
        return result;
    return { ...result, content: state.startedNote + result.content };
}
async function executeKnowCodeToolCore(rawAction, args, workdir, daemonPort = 48123, options = {}, state) {
