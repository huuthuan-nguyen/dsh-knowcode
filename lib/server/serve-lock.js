...
export function readServeLock(dataDir) {
    try {
 ...
 */
export function acquireServeLock(dataDir, workdir, port) {
 ...
 */
export function releaseServeLock(dataDir) {
    const path = serveLockPath(dataDir);
    if (!existsSync(path))
        return false;
    const existing = readServeLock(dataDir);
...
export class DaemonAlreadyRunningError extends Error {
    ...
    staleUnremovable;
    lockPath;
    constructor(existing, workdir, staleUnremovable = false, lockPath) {
        super(staleUnremovable
        ...
        this.staleUnremovable = staleUnremovable;
