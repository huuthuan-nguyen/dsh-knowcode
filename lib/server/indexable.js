...
export const INDEXABLE_GLOB = '**/*.{ts,tsx,js,jsx,mjs,cjs,py,go,rs,java,c,cpp,h,hpp,cs,rb,php,kt,scala,swift,lua,zig,sh,bash,sol,ex,exs,ml,res,m,md,mdx,markdown,txt}';
/**
 ...
 */
export const SCHEMA_EXTENSIONS = new Set([
    '.sql',
    '.cql',
    '.prisma',
    '.proto',
 ...
 */
const SCHEMA_JSON_NAME = /(^|\/)(openapi|swagger|[^/]*\.schema|[^/]*-schema|[^/]*mapping[^/]*|[^/]*elasticsearch[^/]*)\.(json|ya?ml)$/i;
/** Whether a path describes a schema or DDL in source form. */
export function isSchemaPath(relPath) {
    const norm = relPath.replace(/\\/g, '/');
    if (SCHEMA_EXTENSIONS.has(extname(norm).toLowerCase()))
        return true;
 ...
 */
export function isReadableTextPath(relPath) {
    if (isIgnoredPath(relPath))
        return false;
    if (isNonTextPath(relPath))
        return false;
 ...
 */
export function looksBinary(absPath, sniffBytes = 512) {
    try {
...
export const IGNORED_DIR_FRAGMENTS = [
    '/node_modules',
    '/.git',
    '/.knowcode',
    '/dist',
...
export const IGNORE_GLOBS = IGNORED_DIR_FRAGMENTS.map((f) => `**${f}/**`);
/**
...
export function isNonTextPath(relPath) {
    const base = relPath.replace(/\\/g, '/').split('/').pop() ?? '';
    if (SQLITE_SIDECAR.test(base))
        return true;
 ...
 */
export function decideIndexing(absPath, maxFileSize, relPath = absPath) {
    if (isIgnoredPath(relPath))
        return { ok: false, reason: 'ignored' };
    if (isNonTextPath(relPath))
        return { ok: false, reason: 'not-text' };
    if (!isReadableTextPath(relPath))
    ...
    if (looksBinary(absPath))
        return { ok: false, reason: 'binary', size };
    return { ok: true, size };
}
