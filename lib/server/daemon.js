...
import { acquireServeLock, releaseServeLock, serveLockPath, DaemonAlreadyRunningError, } from './serve-lock.js';
import { INDEXABLE_GLOB, IGNORE_GLOBS, SCHEMA_GLOB, DEFAULT_MAX_FILE_SIZE, decideIndexing, } from './indexable.js';
/**
 ...
 */
function removeStrayRdbTemps(dataDir) {
    try {
...
export class KnowCodeDaemon {
     ...
     */
    async waitForWatcherReady(timeoutMs = 10_000) {
        if (!this.watcher)
            return false;
     ...
     */
    async listenOnAvailablePort(requestedPort) {
        ...
        try {
            ...
            this.log(`Port ${requestedPort} is in use (another workspace's daemon?); listening on ${fallbackPort} instead.`);
    ...
    async startInner(startOpts) {
        ...
        if (!lock.acquired) {
            throw new DaemonAlreadyRunningError(lock.existing ?? null, workdir, lock.staleUnremovable === true, serveLockPath(dataDir));
        }
        ...
        if (this.tracer.enabled) {
            ...
            this.tracer.line(`opened index: ${opened.totalFiles} files, ${opened.totalSymbols} symbols, ${opened.totalCalls} calls, ${opened.totalDocs} docs`);
        }
        ...
        if (this.tracer.enabled) {
            ...
            this.tracer.line(`serve ready in ${fmtMs(process.hrtime.bigint() - startupNs)}. ` +
    ...
    startIdleTimer() {
        ...
        this.idleTimer = setInterval(() => {
            ...
            if (this.isIndexing)
                return;
            if (Date.now() - this.lastActivityAt < budget)
                return;
     ...
     */
    async reconcileStartup(opts = {}) {
        if (this.indexingPromise) {
            await this.indexingPromise;
            if (this.lastReconcile)
                return this.lastReconcile;
        }
        let summary = { mode: 'up-to-date', filesIndexed: 0, symbolsIndexed: 0, timeMs: 0 };
        const run = this.runIndexing(async () => {
            const startNs = process.hrtime.bigint();
            const watcherReady = await this.waitForWatcherReady();
            if (!watcherReady)
                this.tracer.line('watcher did not report ready; reconciling anyway');
            if (opts.forceIndex) {
            ...
            try {
                ...
                this.log(`Index stale: ${stale.added.length} added, ${stale.changed.length} changed, ` +
    ...
    async performFullIndexUnlocked(targetPath) {
        this.failedFiles = 0;
        ...
        try {
            ...
            this.tracer.line(`index: scan=${fmtMs(scanMs)} parse+ingest=${fmtMs(parseNs)} relink=${fmtMs(relMs)} ` +
     ...
     */
    async ingestCodeSchema(relPath, content, imports) {
    ...
    async ingestSchemaContent(relPath, content) {
        ...
        if (entities.length + containers.length > 0) {
            this.log(`Schema ${relPath}: ${entities.length} contract entity(ies), ${containers.length} storage container(s).`);
        }
    ...
    async staleCheck(targetPath) {
        ...
        this.tracer.line(`ignore matcher built from stale walk in ${fmtMs(ignoreNs)} (${onDisk.length} candidate files)`);
        const indexedRes = await this.repo.query(`MATCH (f:File) RETURN f.path AS path, f.hash AS hash, f.mtimeMs AS mtimeMs`);
        const indexed = new Map();
        for (const row of (indexedRes.data ?? [])) {
        ...
        if (added.length === 0 && changed.length === 0 && deleted.length === 0) {
            ...
            this.tracer.line(`stale check: ${added.length} added / ${changed.length} changed / ${deleted.length} deleted ` +
     ...
     */
    async reindexPaths(changed, deleted = [], targetPath) {
        if (!this.repo)
            throw new Error('Repository not initialized');
        ...
        this.lastIndexedAt = new Date().toISOString();
        this.tracer.line(`reindex: ${updated} file(s) refreshed, ${deleted.length} removed in ${fmtMs(process.hrtime.bigint() - startNs)}`);
    ...
    async dispatchRpc(action, params) {
        ...
        try {
            ...
            if (action === 'search_symbols') {
                ...
                this.tracer.line(`search: pattern="${pattern}" case_insensitive=true ` +
                ...
                const matches = Array.isArray(result) ? result.length : 0;
                this.tracer.line(`knowledge search: query="${params.query ?? ''}" matches=${matches} elapsed=${fmtMs(elapsedNs)}`);
            }
            return result;
        }
        catch (err) {
            this.tracer.line(`rpc: action=${action} FAILED in ${fmtMs(process.hrtime.bigint() - startNs)}: ${err?.message ?? String(err)}`);
            throw err;
        }
    }
    async dispatchRpcInner(action, params, metrics) {
        if (!this.repo)
            throw new Error('Repository not initialized');
        switch (action) {
            case 'explore':
            ...
            case 'search_symbols':
                return await this.repo.searchSymbols(params.pattern ?? params.query, params.kind, params.limit ?? 25, metrics);
            case 'unused_symbols':
