...
export class CodeParser {
     ...
     */
    static async parseFileAsync(filePath, content) {
    ...
    static parseFile(filePath, content) {
        ...
        if (tree) {
            const astResult = parseWithTreeSitterAst(filePath, lang, source, lines, tree, (file, path) => CodeParser.resolveRelativeCandidates(file, path));
     ...
     */
    static stripStringLiterals(line, inTemplate, inBlockComment = false) {
        let code = '';
        let i = 0;
        let mode = inBlockComment
            ? 'block'
            : inTemplate
                ? 'template'
                : 'none';
     ...
     * itself.
     */
    static codeAfterHeader(code) {
        const brace = code.indexOf('{');
        if (brace >= 0)
            return code.slice(brace);
        const arrow = code.indexOf('=>');
        if (arrow >= 0)
            return code.slice(arrow + 2);
        return '';
    }
    /**
     ...
     */
    static collectCalls(text, callerId, file, lineNum, calls) {
        if (text.length === 0)
            return;
        for (const m of text.matchAll(CALL_PATTERN)) {
            const obj = m[1];
            const fn = m[2];
            if (NON_DECLARATION_KEYWORDS.has(fn))
                continue;
     ...
     */
    static findLibraryAliases(lines, imports) {
        ...
        for (const imp of imports) {
            ...
            for (const raw of lines) {
                ...
                if (bindings.includes(target))
                    continue;
                if (!bindings.some((b) => CodeParser.containsWord(rhs, b)))
                    continue;
     ...
     */
    static gatherHeader(lines, startIdx, maxLookahead = 40) {
        const parts = [];
        let paren = 0;
        let angle = 0;
        let endIdx = startIdx;
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (currentClass) {
                ...
                const methodMatch = classBraceDepth === 1
                    ? codeOnly.match(/^((?:(?:public|private|protected|static|abstract|override|readonly|declare|async|get|set)\s+)*)(\w+)\s*\(/)
                    : null;
                if (methodMatch && !NON_DECLARATION_KEYWORDS.has(methodMatch[2])) {
                    ...
                    currentDoc = [];
                    CodeParser.collectCalls(CodeParser.codeAfterHeader(codeOnly), `${file}:${qname}:${lineNum}`, file, lineNum, calls);
                }
            ...
            if (fnMatch) {
                ...
                CodeParser.collectCalls(CodeParser.codeAfterHeader(codeOnly), `${file}:${name}:${lineNum}`, file, lineNum, calls);
                currentDoc = [];
                continue;
            }
