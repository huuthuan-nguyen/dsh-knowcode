export class StorageParser {
     ...
     */
    static parseGraphqlSdl(file, content) {
        const entities = [];
     ...
     */
    static parseSchemaFile(file, content) {
    ...
    static parseProtobuf(file, content) {
        const entities = [];
        ...
        while ((match = messageRegex.exec(content)) !== null) {
            ...
            for (const line of fieldLines) {
                ...
                const fieldMatch = trimmed.match(/^(?:(required|optional|repeated)\s+)?([A-Za-z0-9_.]+)\s+([A-Za-z0-9_]+)\s*=\s*(\d+)/);
                if (fieldMatch) {
     ...
     */
    static parseXmlXsd(file, content) {
        const entities = [];
        ...
        while ((match = complexTypeRegex.exec(content)) !== null) {
            ...
            const body = match[2];
            const fields = [];
            const elemMatches = body.matchAll(/<xs:element\s+name="([A-Za-z0-9_]+)"(?:\s+type="([A-Za-z0-9_:]+)")?(?:[^>]*?(?:minOccurs="(\d+)")?)?(?:[^>]*?(?:maxOccurs="([^"]+)")?)?[^>]*\/>/g);
     ...
     */
    static parseOpenApiOrJsonSchema(file, content) {
        const entities = [];
        try {
            ...
            for (const [schemaName, schemaObj] of Object.entries(schemas)) {
                ...
                const requiredList = new Set(Array.isArray(schemaObj.required) ? schemaObj.required : []);
                const fields = [];
     ...
     */
    static parseSqlDdl(file, content) {
        const containers = [];
     ...
     */
    static parseMongoSchema(file, content) {
        const containers = [];
        ...
        while ((match = schemaRegex.exec(content)) !== null) {
            ...
            while ((pMatch = propRegex.exec(body)) !== null) {
                ...
                if (propName === 'type' || propName === 'required')
                    continue;
     ...
     */
    static parseElasticMapping(file, content) {
        const containers = [];
        try {
            const parsed = JSON.parse(content);
            ...
            const properties = parsed.mappings?.properties ?? (looksLikeMapping ? parsed.properties : undefined);
     ...
     */
    static matchFieldToAttribute(contractField, storageAttr, engine) {
        ...
        else if ((normC === 'id' && normS.endsWith('_id')) ||
