...
export class DocParser {
     ...
     */
    static parseDoc(filePath, content) {
        if (!DocParser.isDocFile(filePath))
            return null;
        ...
        const sections = [];
        const rules = [];
        const referencedSymbolsSet = new Set();
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (headingMatch) {
                ...
                if (currentSection) {
                    ...
                    for (const s of secSymbols)
                        referencedSymbolsSet.add(s);
                    sections.push({
            ...
            if (trimmed.includes('MUST') ||
                ...
                trimmed.startsWith('- **Rule')) {
                let priority = 'should';
                if (trimmed.includes('MUST') || trimmed.includes('NEVER'))
                    priority = 'must';
                else if (trimmed.includes('AVOID') || trimmed.includes('SHOULD NOT'))
                    priority = 'avoid';
                rules.push({
     ...
     */
    static extractReferencedSymbols(text) {
