...
export async function runServeCommand(targetDir = '.', options = {}) {
    ...
    let summary;
    try {
        summary = await daemon.reconcileStartup({ forceIndex: options.forceIndex });
    }
    catch (err) {
        // Leave nothing half-served: a crashed start used to leave `daemon.json` and
        // `serve.lock` behind, so the next attempt reclaimed a dead guard and failed the
        // same way, making a single bad file look like a permanently broken workspace.
        console.error(`[KnowCode] Indexing failed; stopping the daemon: ${err instanceof Error ? err.message : String(err)}`);
