...
export class KnowCodeRepository {
    ...
    runExclusive(key, task) {
        ...
        void tail.then(() => {
            if (this.ingestQueues.get(key) === tail)
                this.ingestQueues.delete(key);
        });
        return next;
    }
    /**
    ...
    async deleteFile(filePath) {
        await this.graph.query(`MATCH (s:Symbol {file: $filePath}) DETACH DELETE s`, { params: { filePath } });
        await this.graph.query(`MATCH (f:File {path: $filePath}) DETACH DELETE f`, { params: { filePath } });
    }
    /**
     ...
     */
    async deleteSchemaDefinitions(filePath) {
        await this.graph.query(`MATCH (c:ContractEntity {file: $filePath}) DETACH DELETE c`, { params: { filePath } });
        await this.graph.query(`MATCH (sc:StorageContainer {file: $filePath}) DETACH DELETE sc`, { params: { filePath } });
    }
    /**
    ...
    async deleteDoc(docPath) {
        await this.graph.query(`MATCH (sec:DocSection {documentPath: $docPath}) DETACH DELETE sec`, { params: { docPath } });
        await this.graph.query(`MATCH (r:Rule {sourceDoc: $docPath}) DETACH DELETE r`, { params: { docPath } });
        await this.graph.query(`MATCH (d:Document {path: $docPath}) DETACH DELETE d`, { params: { docPath } });
    }
    /**
    ...
    async ingestCodeFileUnlocked(parsed) {
    ...
    async ingestHeritage(heritage) {
        for (const h of heritage) {
            ...
            const resolved = await this.graph.query(`MATCH (sub:Symbol)
    ...
    async ingestDocFileUnlocked(doc) {
    ...
    async explore(limit = 25) {
       ...
       LIMIT $limit`, { params: { limit } });
        ...
        const interfaceMembersRes = await this.graph.query(`MATCH (i:Symbol {kind: 'interface'})
       MATCH (m:Symbol)
       WHERE m.qname = i.name + '.' + m.name
       RETURN DISTINCT m.qname AS qname`);
        const interfaceMembers = new Set((interfaceMembersRes.data ?? []).map((r) => r.qname).filter(Boolean));
       ...
       LIMIT 25`);
        const entryPoints = (entryRes.data ?? [])
            .filter((row) => !interfaceMembers.has(row.qname))
            .slice(0, 15)
            .map((row) => ({
            name: row.name,
            file: row.file,
    ...
    async getPortingContract(symbolName) {
        ...
        if (sym.kind === 'class' || sym.kind === 'interface') {
            const memberCallsRes = await this.graph.query(`MATCH (m:Symbol)-[r:CALLS]->(target)
         ...
         ORDER BY callCount DESC`, { params: { prefix: `${sym.name}.`, self: sym.qname } });
            memberCallees = (memberCallsRes.data ?? []).map((r) => ({
                calleeName: r.name,
       ...
       RETURN r.title AS title, r.content AS content`, { params: { name: sym.name } });
        ...
        const superRes = await this.graph.query(`MATCH (s:Symbol {name: $name})-[:EXTENDS|IMPLEMENTS]->(sup)
       RETURN sup.name AS name, labels(sup) AS labels`, { params: { name: sym.name } });
        const supertypes = (superRes.data ?? []).map((r) => ({
            name: r.name,
        ...
        const importRes = await this.graph.query(`MATCH (f:File {path: $file})-[:IMPORTS]->(target)
       RETURN target.path AS path, f.path AS from
       LIMIT 50`, { params: { file: sym.file } });
    ...
    async getKnowledgeDoc(pathOrTitle) {
       ...
       LIMIT 1`, { params: { query: pathOrTitle } });
    ...
    async findSubtypesAndImplementations(symbolName) {
        const res = await this.graph.query(`MATCH (sub:Symbol)-[r:EXTENDS|IMPLEMENTS]->(target)
     ...
     */
    async findUnusedSymbols(limit = 50, includeExported = false) {
       ...
       LIMIT $limit`, { params: { limit, includeExported } });
        ...
        const contractRes = await this.graph.query(`MATCH (owner:Symbol)-[:IMPLEMENTS|EXTENDS]->(sup:Symbol)
       MATCH (m:Symbol)
       WHERE m.qname = sup.name + '.' + m.name
       RETURN DISTINCT owner.name + '.' + m.name AS qname`);
        const contractMembers = new Set((contractRes.data ?? []).map((r) => r.qname).filter(Boolean));
        return (res.data ?? [])
            .filter((r) => !contractMembers.has(r.qname ?? r.name))
            .map((r) => ({
            name: r.name,
            qname: r.qname ?? r.name,
            kind: r.kind,
    ...
    async generateCrossParadigmBlueprint(symbolName, targetLanguage = 'rust') {
        const sym = await this.getSymbolDefinition(symbolName);
        const supertypes = await this.graph.query(`MATCH (sub:Symbol)-[r:EXTENDS|IMPLEMENTS]->(sup)
    ...
    async extractThirdPartyUsageSlice(libraryPrefix, targetLanguage = 'rust') {
       ...
       RETURN r.importedPath AS path, r.specifiers AS specifiers, f.path AS filePath`, { params: { prefix: libraryPrefix } });
        ...
        for (const row of (importRes.data ?? [])) {
            const set = bindingsByFile.get(row.filePath) ?? new Set();
        ...
        const aliasRes = await this.graph.query(`MATCH (f:File) WHERE f.libraryAliases IS NOT NULL AND f.libraryAliases <> ''
       RETURN f.path AS path, f.libraryAliases AS aliases`, {});
        const matchesLibrary = (modulePath) => modulePath === libraryPrefix ||
        ...
        for (const row of (aliasRes.data ?? [])) {
            ...
            for (const entry of String(row.aliases).split(',')) {
                ...
                if (!alias || !modulePath)
                    continue;
                if (matchesLibrary(modulePath.trim()))
                    set.add(alias.trim());
            }
            if (set.size > 0)
                aliasesByFile.set(row.path, set);
        }
       ...
       RETURN r.calleeName AS name, r.calleeQName AS qname, caller.file AS callerFile, r.line AS line`, { params: { files: Array.from(fileSet) } });
        ...
        for (const row of (callsRes.data ?? [])) {
            ...
            const onBinding = bindings.has(name) ||
